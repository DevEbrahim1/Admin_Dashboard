        ...
        try {
          set_res({
           ...
           loader : true
            ...
            const {data} =await axios.put(`${base_url}/api/news/status_update/${news_id}`,
              {status},{
                headers : {
                    'Authorization' : `Bearer ${store.token}`
                }
    ...
    <div>
          ...
          <tbody>
            {
              ...
              <td className="px-6 py-4">{n.date}</td>
             {
              store?.userInfo?.role === 'admin' ?  <td className="px-6 py-4">
                {
                  n.status === 'pending' && <span onClick={()=>update_status('active',n._id)} className="px-2 py-[2px] bg-green-100 text-blue-800 rounded-lg text-xs cursor-pointer">
                  ...
                  {res.loader && res.id === n._id ? 'Loading...' : n.status}
                   ...
                   {
                  n.status === 'deactive' && <span onClick={()=>update_status('active',n._id)} className="px-2 py-[2px] bg-green-100 text-red-800 rounded-lg text-xs cursor-pointer">
                  {res.loader && res.id === n._id ? 'Loading...' : n.status}
                </span>
                }
              </td> :  <td className="px-6 py-4">
                {
                  n.status === 'pending' && <span className="px-2 py-[2px] bg-green-100 text-blue-800 rounded-lg text-xs cursor-pointer">
                  {n.status}
                </span>
                }
                   {
                  ...
                  {n.status}
                   ...
                   {
                  n.status === 'deactive' && <span className="px-2 py-[2px] bg-green-100 text-red-800 rounded-lg text-xs cursor-pointer">
                  {n.status}
                </span>
                }
              </td>
      ...
      <div className="flex items-center gap-x-3">
        ...
        <IoIosArrowForward onClick={()=>{
          if(page < pages) setPage(page + 1)}} className="w-5 h-5 cursor-pointer"/>
