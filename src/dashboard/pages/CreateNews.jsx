...
const CreateNews = () => {
 ...
 const get_images = async () => {
  try {
    ...
    console.log('GET_IMAGES ERROR 👉', error.response?.data || error.message)
    toast.error(error.response?.data?.message || 'Image load failed')
  }
}
           ...
           <div className="flex flex-col gap-y-2 mb-6">
              ...
              <div className='mb-6'>
                 <div>
                          <label
                            htmlFor="img"
                            className="w-full h-[240px] flex rounded text-[#040404] gap-2 justify-center items-center cursor-pointer border-2 border-dashed"
                          >
                            {
                          ...
                          <input  onChange={imageHandle} className="hidden" type="file" id="img" name="image"  />
                        </div>
              </div>
                <div className="flex flex-col gap-y-2 mb-6">
            ...
            className="px-3 py-[6px] mt-4 bg-purple-500 rounded-sm text-white hover:bg-purple-600"
